...
class NeuralNet:
    ...
    def gradient(self, training_example):
        ...
        for a, w, z, n_w, n_b in zip(self.activations[-2::-1], self.weights[::-1], z[::-1],
            ...
            nabla_w = np.outer(a, sigmoid_derivative(z) * nabla_a)    ###############!!!!!!!!!!!!!! da stimmt was nicht!
...
small_network = NeuralNet(layer_sizes=[3,3,3,2,1])
# small_network.learn(learning_rate=0.5, epochs=1, training_inputs=training_inputs, training_outputs=training_outputs)
small_network.think(np.array([[1,1,1]]))
# print(small_network.weights)
print(small_network.gradient(1))
